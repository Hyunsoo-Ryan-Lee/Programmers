...
테스트 케이스
instructions = ["RIGHT"]리턴(정답): 90
오른쪽으로 90도를 회전하면 방위각은 90이 된다.

instructions = ["LEFT","LEFT","TURN AROUND"]리턴(정답): 0
왼쪽으로 90도 회전하면 로봇은 서쪽을 바라보게 된다 (방위각 270). 다시 왼쪽으로 90도 회전하면 로봇은 남쪽을 바라보게 된다 (방위각 180). 마지막으로 반 바퀴를 회전하면 로봇은 북쪽을 바라보게 된다 (방위각 0).

instructions = ["LEFT 5","RIGHT 10","LEFT 15","RIGHT 20","LEFT 25","RIGHT 30","LEFT 35","RIGHT 40"]리턴(정답): 20
로봇은 왼쪽으로 5도 회전, 오른쪽으로 10도 회전 함으로써 결국 오른쪽으로 5도씩 4번 움직이게 된다.

instructions = ["RIGHT 59","RIGHT","RIGHT","HALT","LEFT","LEFT","LEFT"]리턴(정답): 239
네 번째 명령은 HALT다. 즉 다음에 등장하는 LEFT 명령어들은 실행되지 않는다.

instructions = ["TURN AROUND","HALT","LEFT 5","HALT","LEFT 5","HALT"]리턴(정답): 180
